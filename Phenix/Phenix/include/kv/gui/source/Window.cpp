...
#pragma pack(pop)
        ...
        LRESULT CALLBACK Window::TempWindowProcedure(HWND window_handle, UINT message, WPARAM wparam, LPARAM lparam)
        {
            if (message == WM_NCCREATE)
            {
                ...
                thunk->Build(reinterpret_cast<size_t>(app), reinterpret_cast<size_t>(&Window::AuxWindowProcedure));
        ...
        bool Window::Initialize()
        {
            ...
            if (!RegisterWindowClass(class_name, Window::TempWindowProcedure))
            {
                GetLastError();
        ...
        inline bool Window::RegisterWindowClass(PCTSTR const class_name, WNDPROC window_procdure) const noexcept
        {
